...
export const applyScheduleValidation = (values) => {
  ...
  const validationValues = {
    ...values,
    startDate: values.startDate ? validateDayjsDate(values.startDate) : undefined,
    endDate: values.endDate ? validateDayjsDate(values.endDate) : undefined,
    classTime: values.classTime ? validateDayjsDate(values.classTime) : undefined,
    subjectPeriod: values.subjectPeriod ? validateDayjsDate(values.subjectPeriod) : undefined,
  };
  
  return ScheduleFormSchema(true).validate(validationValues, { abortEarly: false });
};
...
export const TrainingPlanSchema = Yup.object({
  ...
  planLevel: Yup.number()
    ...
    .oneOf([0, 1, 2], "Plan level must be one of: Initial (0), Recurrent (1), Relearn (2)"),
