...
const CreateScheduleForClassPage = () => {
  ...
  const handleCreateScheduleSubmit = async () => {
    try {
      ...
      message.error("Please fill all required fields for schedule.", error);
    } finally {
      setSubmittingStep1(false);
    }
  };
  ...
  /*************  ✨ Windsurf Command ⭐  *************/
  /**
   * Handles the submission process for assigning trainees to a schedule.
   *
   * This function first checks if a ClassSubject ID exists, as it is needed to proceed.
   * Depending on the trainee assignment method ('import' or manual entry), it either
   * imports trainees from an Excel file or assigns manually entered trainees.
   *
   * For 'import':
   * - Ensures a file is selected, then processes the Excel file to import trainees.
   *
   * For manual entry:
   * - Validates the trainee form fields.
   * - Builds assignment objects for each trainee and assigns them.
   *
   * Displays success, error, or warning messages based on the operation outcome and
   * resets the page for a new schedule cycle if all operations are successful.
   *
   * @throws Will display error messages if the assignment fails at any step.
   */

  /*******  2513652a-a29b-4f89-b045-3c5b7439d720  *******/
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-white to-cyan-100 p-4 sm:p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 sm:p-8">
              ...
              >
                <Step
                  ...
                  icon={
                    isStep1Completed ? (
                      <CheckCircleFilled
                        style={{ fontSize: "24px", color: "#14b8a6" }} // cyan-500
                      />
                    ) : (
          ...
          <Card
            className={`shadow-lg rounded-lg mb-8 transition-all duration-500 ${
              isStep1Completed
                ? "opacity-70 border-cyan-500"
                : "border-cyan-600"
            }`}
            bordered
            title={
                ...
                <div className="flex items-center">
                  <SolutionOutlined
                    className={`!mr-3 !text-2xl ${
                      isStep1Completed ? "!text-green-500" : "!text-cyan-600"
                    }`}
                  />
                  <Title
                    level={4}
            ...
            >
              ...
              >
                <Row gutter={24}>
                  ...
                  <Col xs={24} md={24}>
                    <Title level={5} className="!mb-3 !text-cyan-600">
                      <BookOutlined className="!mr-2 !text-cyan-600" />
                      Selection
                    </Title>
                    ...
                    >
                      <Select
                        ...
                        style={{ width: "100%" }}
                        disabled={
                          createdClassSubjectId !== null || submittingStep1
                        }
                      >
                        {subjectSpecialties.map((s) => (
                          <Option
                            key={s.subjectSpecialtyId}
                            value={s.subjectSpecialtyId}
                          >
                            {s.subjectName ||
                      ...
                      <Alert
                        ...
                        description={
                          <Space direction="vertical" style={{ width: "100%" }}>
                            {conflictMessages.map((msgComponent, idx) => (
                              <Paragraph
                                key={idx}
                                style={{
                                  fontSize: "12px",
              ...
              >
                ...
                <div className="flex justify-end mt-6 pt-6 border-t border-gray-200">
                  ...
                  <Button
                    ...
                    size="large"
                    className="!bg-cyan-600 hover:!bg-cyan-700"
                    disabled={!isStep1Completed || submittingStep1}
                  >
