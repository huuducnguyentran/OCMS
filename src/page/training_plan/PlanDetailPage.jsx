...
import { Card, Table, Tag, Empty, Spin, Button, Typography, Breadcrumb, Statistic, Row, Col, Divider, message, Tooltip } from 'antd';
import { 
  ArrowLeftOutlined, 
  CalendarOutlined, 
  BookOutlined, 
...
const PlanDetailPage = () => {
  ...
  console.log("All session storage items:", Object.keys(sessionStorage).map(key => `${key}: ${sessionStorage.getItem(key)}`));
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      message.loading({ content: 'Đang tạo PDF, vui lòng đợi...', key: 'exportPdf' });
      ...
      // Table headers
      const tableHeaders = printContainer.querySelectorAll('.ant-table-thead th');
      ...
      tags.forEach(tag => {
        ...
        const className = Array.from(tag.classList).find(cls => cls.startsWith('ant-tag-'));
        let bgColor = '#f0f0f0';
      ...
      message.success({ content: 'PDF đã được tạo thành công!', key: 'exportPdf' });
    } catch (error) {
      console.error('Lỗi khi xuất PDF:', error);
      message.error({ content: 'Có lỗi xảy ra khi tạo PDF: ' + error.message, key: 'exportPdf' });
    } finally {
