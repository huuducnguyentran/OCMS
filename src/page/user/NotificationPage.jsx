...
import { List, Typography, Badge, Card, message, Spin, Tag, Button, Empty, Modal, Pagination, Alert, Input, Select, Space, Radio } from "antd";
import { notificationService } from "../../services/notificationService";
import { CheckOutlined, ClockCircleOutlined, BellOutlined, CloseCircleOutlined, SearchOutlined, FilterOutlined, SortAscendingOutlined, SortDescendingOutlined } from "@ant-design/icons";
...
const NotificationPage = () => {
  ...
  const applyFiltersAndSearch = () => {
    ...
    if (filterTypes.length > 0) {
      result = result.filter(item => filterTypes.includes(item.notificationType));
    }
  ...
  const updatePaginatedData = () => {
    ...
    const paginatedData = filteredNotifications.slice(startIndex, startIndex + pageSize);
  ...
  const handleMarkAsRead = async (notificationId, e) => {
    ...
    try {
      ...
      message.error("Cannot update notification: " + (error.response?.data?.message || error.message || "Please try again later."));
    }
  };
  ...
  const getUniqueNotificationTypes = () => {
    const types = new Set(allNotifications.map(item => item.notificationType));
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
          ...
          <div className="mt-3 flex justify-end">
            <Radio.Group value={sortOrder} onChange={handleSortOrderChange} buttonStyle="solid">
        ...
        <Spin spinning={loading}>
          {notifications.length > 0 ? (
                ...
                renderItem={(item) => {
                  ...
                  return (
                    <List.Item
                      ...
                      className={`cursor-pointer border-b hover:bg-gray-50 p-4 ${!notificationIsRead ? 'bg-blue-50' : ''}`}
                        ...
                        <List.Item.Meta
                          ...
                          title={
                            <div className="flex items-center">
                              <span className={!notificationIsRead ? 'font-semibold' : ''}>{item.title}</span>
                            </div>
                          }
                          description={
                            <div className="flex flex-col">
                              <span className="text-gray-500 truncate max-w-lg">{item.message}</span>
                              <div className="flex items-center mt-1">
                                <Tag color={getNotificationTypeColor(item.notificationType)}>
                                  {item.notificationType}
                                </Tag>
                                <span className="text-xs text-gray-400 ml-2">{getFormattedDate(item.createdAt)}</span>
                              </div>
                            </div>
                          }
                        />
            ...
            <Empty
              ...
              description={error ? "Unable to load notifications" : filteredNotifications.length > 0 ? "No matching notifications found" : "No notifications"}
            />
          )}
        </Spin>
      </div>
        ...
        {selectedNotification && (
          <div className="notification-detail">
            <div className="mb-4">
              ...
              <Tag color={getNotificationTypeColor(selectedNotification.notificationType)}>
                {selectedNotification.notificationType}
              </Tag>
            </div>
            
            <p className="my-4 text-base whitespace-pre-line">{selectedNotification.message}</p>
